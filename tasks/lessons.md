...
## 38. Синхронизация 3D-плашек над головой и добавление коллайдеров при переходе взаимодействия со стойки на NPC
* **Ошибка**:
  - При переходе логики обслуживания покупателей со стойки (`ServiceCounter`) непосредственно на NPC (`NPCBuyer` через `IInteractable`), на префабе NPC отсутствовал физический коллайдер. Из-за этого луч `PlayerInteraction` проходил сквозь персонажей и утыкался в пол (вызывая промпт грязи `MessSpot` вместо меню обслуживания).
  - Разовый расчет высоты головы `headHeight` через `InverseTransformPoint(t.position).y` в методе `Initialize()` Patience UI происходил до первого кадра аниматора, когда кости персонажа еще не приняли правильные мировые координаты. В результате UI терпения и черная плашка цензуры (3D-оверлей паранойи) рассчитывались с огромным сдвигом по высоте и висели высоко в воздухе.
  - Позиционирование 3D-плашки цензуры по положительной локальной оси Z (`+0.12f`) приводило к тому, что при развороте билборда лицом к камере плашка оказывалась позади головы персонажа. Из-за этого при взгляде сверху вниз под углом она проецировалась выше головы.
* **Последствие**: Игрок не мог обслужить очередь (принять заказ или выдать напиток), а черная полоса цензуры и плашка терпения летали высоко над NPC, нарушая визуал игры.
* **Правило**:
  - При переносе логики взаимодействия на динамические объекты (NPC, переносимые предметы) обязательно убедиться в наличии коллайдера на корневом объекте. При необходимости динамически добавлять `CapsuleCollider` в `Awake()` в режиме триггера (`isTrigger = true`), чтобы не мешать физике NavMesh, но позволять регистрировать рейкасты.
  - Никогда не кэшировать высоту головы по мировым координатам костей в методах инициализации (например, `Initialize()`), так как на момент вызова мировые матрицы дочерних трансформ могут быть не обновлены. Вместо этого кэшировать ссылку на `Transform` кости (например, `Head`) и динамически синхронизировать позицию контейнера UI с позицией кости в `Update()`.
  - Для 3D-билбордов, вращающихся лицом к камере (где локальный `-Z` направлен к камере), размещать лицевые элементы (очки, плашки цензуры перед глазами) по отрицательной оси Z (`-0.12f` or `-0.15f`) и положительному Y офсету относительно кости `Head`, чтобы они всегда проецировались точно поверх лица со стороны взгляда игрока.

## 40. Детекция достижения цели NavMeshAgent и предотвращение преждевременного уничтожения NPC при смене путей
* **Ошибка**: Проверка завершения пути NPC через `_agent.remainingDistance <= _agent.stoppingDistance` на том же кадре, когда задается новый путь (например, вызов `GoToExit()`), приводила к тому, что агент мгновенно уничтожался. В Unity вычисление нового пути NavMeshAgent занимает время, и до завершения расчёта (когда `pathPending` еще ложно или уже снято) `remainingDistance` возвращает старое или нулевое значение.
* **Последствие**: Вместо того чтобы красиво уйти/убежать из паба к выходу, NPC мгновенно растворялся в воздухе у барной стойки.
* **Правило**: При смене пути и проверке достижения конечной точки назначения (например, выхода) сверять мировое расстояние от текущей позиции агента до конечной точки назначения (`Vector3.Distance(transform.position, _agent.destination)`), а не полагаться только на `remainingDistance`, либо дождаться начала валидного движения агента.

## 41. Ограничение перемещений NavMeshAgent сквозь стены здания через динамические NavMeshObstacles на префабах
* **Ошибка**: Попытка заставить NPC обходить стены здания без добавления NavMeshObstacles на внешние стены приводила к тому, что они проходили сквозь стены напрямую к цели, так как NavMesh был запечен по всей площади плоскости пола.
* **Последствие**: NPC игнорировали стены и двери, проходя сквозь физические препятствия.
* **Правило**:
  - Назначать компоненты `NavMeshObstacle` с включенным `carving = true` непосредственно на префабы стен (`Wall_Segment_2m`), колонн (`Pillar_Segment`) и окон (`Window_Segment`), чтобы они автоматически вырезали непроходимые области на сетке NavMesh.
  - Дверные проемы оснащать динамическими `NavMeshObstacle`, которые выключаются при открытии дверей (`navObstacle.enabled = !anyOpen`). Это заставляет NPC обходить здание, когда двери закрыты, но позволяет им входить внутрь и становиться в очередь, как только двери открываются.

## 39. Принудительный сброс стейта NPC (NPCState) при завершении обслуживания или таймауте очереди
* **Ошибка**: При переводе покупателя из очереди в состояние ухода (`_barState = BarCustomerState.Leaving; _isInBarQueue = false;`) его основное состояние `_state` оставалось равным `NPCState.Walking`. Из-за этого в следующем кадре метод `Update()` перенаправлял его в `UpdateWalking()`. Так как NPC уже находился вплотную к стойке (`remainingDistance <= stoppingDistance`), `UpdateWalking()` ошибочно считал, что NPC только что дошел до стойки/стенда, и переводил его в состояние `Evaluating` (оценивание), перезаписывая путь к выходу и заставляя NPC стоять на месте.
* **Последствие**: Покупатели, которые должны были «убежать в страхе» после диалога допроса, успешно завершившие покупку или ушедшие по таймауту терпения, оставались стоять у стойки навсегда.
* **Правило**: При выводе покупателя из очереди бара/обслуживания (установка `_barState = BarCustomerState.Leaving` и `_isInBarQueue = false`) всегда принудительно переключать его основное состояние движения в `_state = NPCState.Leaving`. Это гарантирует, что в следующем кадре управление будет передано методу `UpdateLeaving()`, который без прерываний доведет персонажа до точки выхода из магазина и уничтожит его объект.

## 42. Проверка существования NavMeshObstacle перед добавлением через AddComponent при повторной генерации
* **Ошибка**: Попытка динамически добавить компонент `NavMeshObstacle` с помощью метода `AddComponent<NavMeshObstacle>()` на объект, который уже содержит этот компонент (поскольку он был добавлен на префаб), приводила к ошибке Unity о невозможности добавления дублирующего компонента. Метод возвращал `null`, что вызывало `NullReferenceException` на следующей строке при попытке настроить параметры (например, `obstacle.carving = true`).
* **Последствие**: Процесс пересборки стен здания (`WallRebuilder.cs`) падал с критической ошибкой, и сцена оставалась в некорректном или незавершенном состоянии.
* **Правило**: При динамической настройке компонентов NavMesh на объектах, которые могут наследоваться от префабов, всегда проверять существование компонента с помощью `GetComponent<T>()` перед вызовом `AddComponent<T>()`.
